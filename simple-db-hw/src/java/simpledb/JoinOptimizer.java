...
public class JoinOptimizer {
    ...
    public int estimateJoinCardinality(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
        	...
        	if(j.p == Op.EQUALS)
        	{
        		...
        		return (int) Math.ceil(card1 * card2 * 0.3);
        	}
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	//Create a new PlanCache
		PlanCache plan = new PlanCache();
		
		//For each join in joins, enumerate subsets
		for (int i = 0; i <= this.joins.size(); i++) {
			Set<Set<LogicalJoinNode>> subset = enumerateSubsets(joins, i);
			//For each node in the subset, add the plan if there is one
			for (Set<LogicalJoinNode> node : subset) {
				double c = Double.MAX_VALUE;
				CostCard p = null;
				
				//For each node in the set of nodes, compute the cost of the subplan
				for (LogicalJoinNode l : node) {
					CostCard sub = computeCostAndCardOfSubplan(stats, filterSelectivities, l, node, c, plan);
					if (sub != null && sub.cost < c) {
						c = sub.cost;
						p = sub;
					}
				}
				if (p != null) {
					plan.addPlan(node, c, p.card, p.plan);
				}
			}
		}
		
		Vector<LogicalJoinNode> order = plan.getOrder(new HashSet<LogicalJoinNode>(joins));
		
		if (explain) {
			printJoins(order, plan, stats, filterSelectivities);
		}
