...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if(pid.pgcateg() == BTreePageId.LEAF){
			...
			//Iterate through the entries of this page
			BTreeInternalPage internal = (BTreeInternalPage) this.getPage(tid, dirtypages, pid,	Permissions.READ_ONLY);
			Iterator<BTreeEntry> iterator = internal.iterator();
			BTreeEntry entry = null;
			
			while (iterator.hasNext()) {
				entry = iterator.next();
				//Recurse if the entry's keyfield is greater than f or if f is null
				if (f == null || entry.getKey().compare(Op.GREATER_THAN_OR_EQ, f)) {
					return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
				}
			}
			//Recurse on the entry's rightchild
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage right = (BTreeLeafPage) this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF), temp = page, rOld = null;
		BTreePageId rightId = right.getId(), pageId = page.getId();
		
		int half = (int) Math.ceil((double) temp.getNumTuples() / 2);
		Iterator<Tuple> it = temp.reverseIterator();
		
		//Modify siblings of page if the rightSibling exists already		
		if(temp.getRightSiblingId() != null){
			rOld = (BTreeLeafPage) this.getPage(tid, dirtypages, temp.getRightSiblingId(), Permissions.READ_WRITE);
			rOld.setLeftSiblingId(rightId);
			right.setRightSiblingId(temp.getRightSiblingId());
		}
		
		//Set siblings
		right.setLeftSiblingId(pageId);
		temp.setRightSiblingId(rightId);
		
		//Iterate over half the tuples. Delete from the page and move to the right page
		for(int i = 0; i<half && it.hasNext(); i++){
			Tuple t = it.next();
			temp.deleteTuple(t);
			right.insertTuple(t);
		}
		
		//Get the first tuple on the right leaf page
		Tuple t = right.iterator().next();
		
		//Create internal parent page and entry
		BTreeInternalPage parent = this.getParentWithEmptySlots(tid, dirtypages, temp.getParentId(), t.getField(keyField));
		BTreePageId parentId = parent.getId();
		BTreeEntry entry = new BTreeEntry(t.getField(keyField), pageId, rightId);
		
		//Set ParentIds and insert entry into parent
		temp.setParentId(parentId);
		right.setParentId(parentId);
		parent.insertEntry(entry);
		
		//Update hashMap
		dirtypages.put(pageId, temp);
		dirtypages.put(rightId, right);
		dirtypages.put(parentId, parent);
		
		//Decide which page to return
		if (field.compare(Op.GREATER_THAN_OR_EQ, t.getField(keyField))) {
			return right;
		} else
			return temp;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage internal = (BTreeInternalPage) this.getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		BTreePageId internalId = internal.getId(), pageId = page.getId();
		...
		//foreach entry in the page, delete the key from page and insert into the new internal page
		for (int i = 0; i < half; i++) {
			entry = it.next();
			page.deleteKeyAndRightChild(entry);
			internal.insertEntry(entry);
		}
		//The first entry of the last half of the remaining entries
		entry = it.next();
		...
		//Set children and remove entry from page, move to parent
		page.deleteKeyAndRightChild(entry);
		entry.setLeftChild(pageId);
		entry.setRightChild(internalId);
		parent.insertEntry(entry);
		...
		this.updateParentPointers(tid, dirtypages, internal);

		//Update hashMap
		dirtypages.put(pageId, page);
		dirtypages.put(internalId, internal);
		dirtypages.put(parentId, parent);
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		if (isRightSibling) {
			//right sibling, regular iteration
			it = sibling.iterator();
		} else {
			//Left sibling, reverse iterator
			it = sibling.reverseIterator();
		}

		//Move tuples from the sibling to the page, until the page has more 
		...
		if (isRightSibling) {
			//Set key of entry using next value of it
			entry.setKey(it.next().getField(this.keyField));
		} else {
			//Set key of entry using the next value of page iterator
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//Left sibling, requires reverse iteration
		Iterator<BTreeEntry> it = leftSibling.reverseIterator();
		
		while (page.getNumEntries() < half) {
			//Get the entry and delete it from the left sibling
			BTreeEntry entry = it.next();
			Field f = entry.getKey();
			leftSibling.deleteKeyAndRightChild(entry);
			
			//Set the key of the entry and the parentEntry
			entry.setKey(parentEntry.getKey());
			parentEntry.setKey(f);

			//Update entry in parent and update the children
		...
		//Update the pointers of page and leftSibling
		this.updateParentPointers(tid, dirtypages, page);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		while (page.getNumEntries() < half) {
			//Get entry and delete from sibling
			BTreeEntry entry = it.next();
			Field f = entry.getKey();
			rightSibling.deleteKeyAndLeftChild(entry);
			
			//Set keys of entry and parentEntry
			entry.setKey(parentEntry.getKey());
			parentEntry.setKey(f);

			//Update parent and update children
			parent.updateEntry(parentEntry);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//Get all the ids of the pages
		BTreePageId rightSibId = rightPage.getRightSiblingId(), leftId = leftPage.getId(), parentId = parent.getId();
		
		Iterator<Tuple> it = rightPage.iterator();
		while (it.hasNext()) {
			//Move tuples from right page to left page
			Tuple t = it.next();
			rightPage.deleteTuple(t);
			leftPage.insertTuple(t);
		}
		
		//Set id of rightsibling
		leftPage.setRightSiblingId(rightSibId);
		
		if (rightSibId != null) {
			//Not the end of the tree, so create a left page and link it to the leftPage
			BTreeLeafPage right = (BTreeLeafPage) this.getPage(tid, dirtypages, rightSibId, Permissions.READ_WRITE);
			
			right.setLeftSiblingId(leftId);
		}

		//Update hashMap
		dirtypages.put(parentId, parent);
		dirtypages.put(leftId, leftPage);
		
		//Delete the parententry
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//Insert the entry into the leftPage
		leftPage.insertEntry(parentEntry);
		
		while (it.hasNext()) {
			//Move entries from rightPage to leftPage
			BTreeEntry entry = it.next();
